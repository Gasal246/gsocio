import { useDeleteSavedPost, useGetCurrentUser, useLikedPost, useSavedPost } from "@/lib/react-query/queriesAndMutations"
        ...
        <div className="flex justify-between items-center z-20">
            ...
            <div className="flex gap-2 mr-5">
                <img src={checkIsLiked(likes, userId) ? "/assets/icons/liked.svg" : "/assets/icons/like.svg"}
                    alt="like-icon" width={20} height={20}
                    onClick={handleLikePost} className="cursor-pointer" />
                <p className="small-meium lg:base-medium">{likes.length}</p>
            </div>
            {/* save */}
            <div className="flex gap-2">
                {isSavingPost || isDeletingSaved ? <Loader /> :
                    <img src={isSaved ? "/assets/icons/saved.svg" : "/assets/icons/save.svg"} alt="save-icon"
                        width={20} height={20}
